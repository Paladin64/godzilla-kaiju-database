...
class EditGodzillaClone extends Component {
  ...
  onChangeAppearances(e) {
    let appearances = this.state.appearances.slice();
    appearances[e.target.key] = e.target.value;
    this.setState({
      appearances
    });
  }
  ...
  onChangeAbilities(e) {
    ...
    abilities[e.target.key] = e.target.value;
    this.setState({
      abilities
    });
  }
  ...
  onSubmit(e) {
    ...
    axios
      .put(
        "http://localhost:5000/godzilla_clones/update/" +
    ...
    this.props.history.push("/godzilla-clone-list");
  }
  render() {
    return (
      <>
